...
public class AimController : ListeningMonoBehaviour {
    ...
    IEnumerator findNextTarget() {
        ...
        int layerMask = 1 << 18;
        Vector3 dir2 = engagedPosition - killingCamera.transform.position;
    ...
    private void mosquitoesEngaged(int MosquitoeNumber) {
        if (MosquitoeNumber == -1) mosquitoesEngagedMode = false;
        if (!mosquitoesEngagedMode) {
            mosquitoesEngagedMode = true; 
            engagedPosition = MosquitoSpawner.SharedInstance.getMosquitoPositionByIndex(MosquitoeNumber);
        }
    }
